...
export const createComponent = (
    ...
    if (folder) {
        if (!fs.existsSync(resolve(execPath, folder))) {
            ...
            console.log(chalk.yellow(`---> Message: Folder ${folder} was found. <---`));
            execPath = resolve(execPath, folder);
        }
    }
    ...
    if (!fs.existsSync(resolve(execPath, title))) {
        ...
        console.log(chalk.red(`!!!-> Error: Component ${title} already exists! <-!!!`));
        return;
    }
    ...
    if (addCssExt) {
        fs.appendFile(
            ...
            styleTemplate(title),
            () => console.log(chalk.green("---> Styles file was successfully created! <---"))
        );
        styleType = "css";
    }
        ...
        if (!noIndex) {
            if (!fs.existsSync(resolve(execPath, "index.js"))) {
                fs.appendFile(
                    ...
                    IndexTemplate(title),
                    () =>
                        console.log(chalk.green("---> Index file was successfully created <---"))
                );
            } else {
                const file = fs
                ...
                console.log(chalk.yellow("---> Index file was successfully updated <---"))
            }
        }
    }
        ...
        if (!noIndex) {
            if (!fs.existsSync(resolve(execPath, "index.ts"))) {
                fs.appendFile(
                    ...
                    () =>
                        console.log(
                            chalk.green("---> Ts index file was successfully created <---")
                        )
                );
            } else {
                const file = fs
                ...
                console.log(chalk.yellow("---> TS index file was successfully updated <---"))
            }
        }
